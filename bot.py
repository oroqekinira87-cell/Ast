...
BOT_TOKEN: str = os.getenv("BOT_TOKEN", "8406671676:AAGJvTA2nuKvDGzp4chiffBPdM10rxjFuRk")
...
async def check_subscription(user_id: int, bot) -> Tuple[bool, List[Channel]]:
    if not db.settings.get("require_subscription", True): return True, []
    if is_admin(user_id): return True, []
...
async def enforce_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if ok: return True
    text = (
        f"𓆩🔒𓆪 <b>الاشتراك إجباري</b>\n"
        f"{SEP_MAIN}\n"
        f"للاستمرار يجب الاشتراك في القنوات التالية\nثم اضغط «تحققت من الاشتراك»:"
    )
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if isinstance(context.error, Conflict):
        logger.critical("تعارض في الـ webhook — تأكد من تشغيل نسخة واحدة فقط!")
...
def main():
    ...
    app.post_init = post_init
    # حلقة إعادة تشغيل تلقائية عند انقطاع الشبكة لتفادي توقف البوت
    while True:
        try:
            app.run_polling(
